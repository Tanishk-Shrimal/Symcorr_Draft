...
Lattice quantum chromodynamics (Lattice QCD) provides a first-principles,
non-perturbative formulation of the strong interaction and has become the
standard framework for studying hadron spectroscopy, hadronic matrix elements,
and the interactions of multi-hadron systems. Modern spectroscopy calculations
typically employ large variational operator bases to reliably isolate excited
states, resonances, and near-threshold scattering channels. The quality of the
extracted spectrum therefore depends critically on the construction of
interpolating operators with good overlap onto the physical states of interest
and on the efficient evaluation of the corresponding Euclidean correlation
functions.

Among the available smearing techniques, the distillation framework
introduced in Ref.~\cite{Peardon:2009gh} has emerged as one of the most
successful approaches for spectroscopy calculations. By projecting the quark
fields onto a low-dimensional Laplacian eigenvector basis, distillation
provides a compact representation of quark propagation while preserving the
flexibility to construct large and highly structured operator bases. It has
enabled precision studies of excited hadrons, coupled-channel scattering,
hybrid mesons, and multi-hadron systems that would otherwise be prohibitively
expensive.

Despite these advantages, the software required to construct and evaluate
correlation functions in the distillation framework remains largely
hand-crafted. New operator constructions often require significant manual
implementation effort, including symbolic Wick contractions, flavor and spin
bookkeeping, and the translation of analytical expressions into optimized
tensor contractions. As operator bases become larger and more diverse, this
manual workflow becomes increasingly difficult to maintain, verify, and extend.

This work introduces \textsc{SymCorr}, a compiler framework that automates the
construction and optimization of lattice QCD correlation functions within the
distillation formalism. Rather than treating correlator construction as a
collection of ad hoc tensor contractions, \textsc{SymCorr} represents
interpolating operators symbolically, performs automatic Wick contractions,
identifies reusable computational structures, and generates optimized execution
graphs suitable for efficient numerical evaluation.

\subsection{Challenges in lattice QCD correlator construction}

The numerical evaluation of hadronic correlation functions involves several
distinct stages. Interpolating operators are first constructed with the desired
quantum numbers, including flavor, spin, momentum, and lattice symmetry
transformations. Correlation functions are then obtained by applying Wick's
theorem, producing a sum over quark contractions that must subsequently be
expressed in terms of quark propagators.

Within the distillation formalism these propagators are replaced by
perambulators and mode-space operator kernels. Although this factorization
greatly reduces the computational cost of propagator inversions, it introduces
a new layer of algebraic complexity. The resulting expressions consist of large
collections of tensor contractions whose structure depends on the flavor
content, spin couplings, momentum projections, derivatives, and color
contractions appearing in the interpolating operators.

For realistic spectroscopy calculations involving dozens or hundreds of
operators, manually deriving and implementing these contractions is
time-consuming and prone to error. Small modifications to an operator basis
often require extensive changes throughout the implementation, while redundant
sub-expressions are difficult to identify by hand. These challenges are
particularly severe for multi-hadron systems, where the number of Wick
contractions grows rapidly with the number of quark fields.

Consequently, there is a need for software capable of representing
interpolating operators symbolically, automatically performing Wick
contractions, exploiting the algebraic structure of distillation, and producing
optimized computational graphs for numerical evaluation.
